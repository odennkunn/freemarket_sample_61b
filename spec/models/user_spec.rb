...
#nickname 重複した場合登録できない
    it "is invalid with a duplicate nickname" do
      ...
      another_user.valid?
      expect(another_user.errors[:nickname]).to include("はすでに登録されているニックネームです")
    end
...
#emailが重複した場合登録できない
    it "is invalid with a duplicate email" do
      ...
      another_user.valid?
      expect(another_user.errors[:email]).to include("はすでに登録されているアドレスです")
    end
...
#passwordが7~128文字であるか
  ...
  #passwordが英語のみ
      it "is invalid with a password that only letters" do
        ...
        user.valid?
        expect(user.errors[:password][0]).to include("は英数字両方を含めて入力してください")
      end
  ...
  #passwordとpassword_confirmationが一致していない
      it "is valid with a password and password_confirmation match" do
        ...
        user.valid?
        expect(user.errors[:password_confirmation][0]).to include("のパスワードの入力が一致しません")
      end
  ...
  #kana_family_nameが36文字以上
      it "is invalid with a kana_family_name that has more than 36 characters" do
        ...
        user.valid?
        expect(user.errors[:kana_family_name]).to include("は35文字以内で入力してください")
      end
  ...
  #kana_last_nameが36文字以上
      it "is invalid with a kana_last_name that has more than 36 characters" do
        ...
        user.valid?
        expect(user.errors[:kana_last_name]).to include("は35文字以内で入力してください")
      end
